...
if ($method === 'GET') {
    ...
    if (!$customerId) jsonResponse(['success'=>false, 'message'=>'Login required'], 401);
...
if ($method === 'POST') {
    ...
    if ($action === 'place_order') {
        ...
        try {
            ...
            $db->commit();
            jsonResponse(['success'=>true, 'message'=>'Order placed successfully', 'order_id'=>$orderId, 'order_number'=>$orderNum, 'loyalty_earned'=>$loyaltyPoints ?? 0]);
        } catch (Exception $e) {
            $db->rollBack();
